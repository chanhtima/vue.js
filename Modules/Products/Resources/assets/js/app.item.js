$(document).ready(function() {
  ...
  addColor = function() {
    ...
    var icon =
      '<div id="show-color-' +
      count +
      '" class="circle-badge color-choosen choosen" style="background-color:' +
      code +
      ';"><span class="badge"><ez>' +
      name +
      '</ez> <i onclick="colorInit(' +
      count +
      ',this);" class="fa fa-pencil edit-color pl-2" ></i> <i onclick="deleteColor(' +
      count +
      ',this);" class="fa fa-times-circle delete-color" ></i></span><div class="contain-data-box"></div></div>';
    var input =
      '<input type="hidden" class="color_input" name="color[' +
      count +
      ']" id="input_color_' +
      count +
      '" data-color-id="' +
  ...
  colorInit = function(id, ui) {
    let name = $("#input_color_" + id)
      .val()
      .split(",")[0];
    let code = $("#input_color_" + id)
      .val()
      .split(",")[1];
    let html = "<div>";
    ...
    html += "   </div>";
    html +=
      '    <input type="text" name="edit_color_name_' +
      id +
      '" placeholder="' +
      name +
      '" value="' +
    ...
    html += "   </div>";
    html +=
      '    <input type="color" class="form-control" name="edit_color_code_' +
      id +
    ...
    html +=
      ...
      id +
      ');" class="badge btn btn-primary"><i class="fa fa-plus"></i> Edit</button>';
    html += "       </div>";
    html += "   </div>";
    $("#show-color-" + id + " .contain-data-box").html(html);
    $("#show-color-" + id + " .contain-data-box").addClass(
      "mt-2 mb-2 border rounded p-2"
    );
    $("#show-color-" + id).toggleClass("edit");
  };
  
  editColor = function(id) {
    let name = $('input[name="edit_color_name_' + id + '"]').val();
    let code = $('input[name="edit_color_code_' + id + '"]').val();
    ...
    $("#show-color-" + id + " .contain-data-box").removeClass(
      "mt-2 mb-2 border rounded p-2"
    );
  };
  
  deleteColor = function(id, element) {
    let name = $("#input_color_" + id)
      .val()
      .split(",")[0];
    bootbox.confirm({
  ...
  addSize = function() {
    ...
    var icon =
      '<div id="show-size-' +
      count +
      '" class="circle-badge size-choosen choosen"><span class="badge"><ez>' +
      name +
      '</ez> <i onclick="sizeInit(' +
      count +
      ',this);" class="fa fa-pencil edit-size pl-2" ></i> <i onclick="deleteSize(' +
      count +
      ',this);" class="fa fa-times-circle delete-color" ></i></span><div class="contain-data-box"></div></div>';
    var input =
      '<input type="hidden" class="size_input" name="size[' +
      count +
      ']" id="input_size_' +
      count +
      '" data-size-id="' +
  ...
  sizeInit = function(id, ui) {
    let name = $("#input_size_" + id)
      .val()
      .split(",")[0];
    let html = '<div class="form-group input-group mb-2 size-name">';
    html += '   <div class="input-group-prepend">';
    html += '     <span class="input-group-text" id="basic-addon1">size</span>';
    html += "   </div>";
    html +=
      '       <input type="text" class="form-control m-0" name="edit_size_name_' +
      id +
      '" placeholder="' +
      name +
      '" value="' +
    ...
    html +=
      '       <button type="button" onclick="editSize(' +
      id +
      ');" class="badge btn btn-primary"><i class="fa fa-plus"></i> Edit</button>';
    html += "   </div>";
    $("#show-size-" + id + " .contain-data-box").html(html);
    $("#show-size-" + id + " .contain-data-box").addClass(
      "mt-2 mb-2 border rounded p-2"
    );
  ...
  editSize = function(id) {
    let name = $('input[name="edit_size_name_' + id + '"]').val();
    ...
    $("#show-size-" + id + " .contain-data-box").removeClass(
      "mt-2 mb-2 border rounded p-2"
    );
  };
  
  deleteSize = function(id, element) {
    let name = $("#input_size_" + id)
      .val()
      .split(",")[0];
    bootbox.confirm({
  ...
  addNewImage = function() {
    ...
    html +=
      '<input type="text" class="form-control" name="uploadfilesName_th[' +
      count +
      ']" placeholder="ชื่อภาพ" value="">';
    html += '<label class="form-label pt-2">ชื่อภาพ (EN)</label>';
    html +=
      '<input type="text" class="form-control" name="uploadfilesName_en[' +
      count +
      ']" placeholder="ชื่อภาพ" value="">';
    html +=
      '<button type="botton" class="btn btn-danger btn-remove-image"><i class="fa fa-trash"></i></button>';
    html += "</div>";
  ...
  createManagePrice = function(
    ...
    if (
      temp_price.find("#discount_price_" + color_id + "_" + size_id).length > 0
    ) {
      dc = $("#discount_price_" + color_id + "_" + size_id).val();
    }
    if (
      temp_price.find("#member_normal_price_" + color_id + "_" + size_id).length >
      0
    ) {
      mnp = $("#member_normal_price_" + color_id + "_" + size_id).val();
    }
    if (
      temp_price.find("#member_discount_price_" + color_id + "_" + size_id)
        .length > 0
    ) {
      mdc = $("#member_discount_price_" + color_id + "_" + size_id).val();
    }
    table_price =
      '<tr class="row_manage_price" id="row_manage_price_' +
      color_id +
      "_" +
      size_id +
      '">';
    ...
    table_price +=
      '<input type="number" min="0" class="form-control normal_price" placeholder="ราคาปกติ" name="normal_price_' +
      color_id +
      "_" +
      size_id +
      '" id="normal_price_' +
    ...
    table_price +=
      '<input type="number" min="0" class="form-control discount_price" placeholder="ส่วนลดปกติ" name="discount_price_' +
      color_id +
      "_" +
      size_id +
      '" id="discount_price_' +
    ...
    table_price += "</td>";
    table_price +=
      '<td class="manage_discount_type">' + dc_txt[discount_type] + "</td>";
    table_price += "<td>";
    table_price += '<div class="form-group frm-mnPrice">';
    table_price +=
      '<input type="number" min="0" class="form-control member_normal" placeholder="ราคาสมาชิก" name="member_normal_price_' +
      color_id +
      "_" +
      size_id +
      '" id="member_normal_price_' +
      color_id +
      "_" +
      size_id +
      '" value="' +
    ...
    table_price +=
      '<input type="number" min="0" class="form-control member_discount" placeholder="ส่วนลดสมาชิก" name="member_discount_price_' +
      color_id +
      "_" +
      size_id +
      '" id="member_discount_price_' +
      color_id +
      "_" +
      size_id +
      '" value="' +
    ...
    table_price += "</td>";
    table_price +=
      '<td class="manage_member_discount_type">' +
      dc_txt[member_discount_type] +
      "</td>";
  ...
  setMemberDiscountType = function(val) {
    let dc_txt = {
      0: "จำนวนเงิน",
      1: "เปอร์เซ็น",
    };
  ...
  createManageStock = function(
    ...
    table_stock =
      '<tr class="row_manage_stock" id="row_manage_stock_' +
      color_id +
      "_" +
      size_id +
      '">';
    ...
    table_stock +=
      '<input type="text" class="form-control" placeholder="SKU" name="sku_' +
      color_id +
      "_" +
      size_id +
      '" id="sku_' +
    ...
    table_stock +=
      '<input type="text" class="form-control" placeholder="Barcode" name="barcode_' +
      color_id +
      "_" +
      size_id +
      '" id="barcode_' +
    ...
    table_stock +=
      '<input type="number" min="0" class="form-control" placeholder="จำนวนสินค้า" name="quantity_' +
      color_id +
      "_" +
      size_id +
      '" id="quantity_' +
  ...
  pickVendor = function(value) {
    ...
    if ($("#vendors").val() == "") {
      ...
      let show_vendor =
        '<span class="tag" > ' +
        new_vendor[1] +
        " <i onclick=\"deleteVendor('" +
        new_vendor[1] +
        "'," +
        new_vendor[0] +
        ',this);" class="fa fa-times-circle delete-vendor" ></i></span>';
  ...
  deleteVendor = function(value, id, element) {
    bootbox.confirm({
      size: "small",
  ...
  pickLabel = function(value) {
    ...
    if ($("#labels").val() == "") {
      ...
      let show_label =
        '<span class="tag" > ' +
        new_label[1] +
        " <i onclick=\"deleteLabel('" +
        new_label[1] +
        "'," +
        new_label[0] +
        ',this);" class="fa fa-times-circle delete-label" ></i></span>';
  ...
  deleteLabel = function(value, id, element) {
    bootbox.confirm({
      size: "small",
  ...
  addNewTag = function(tag, lang) {
    ...
    if ($("#tags_" + lang).val() == "") {
      ...
      let show_tag =
        ...
        "','" +
        tag +
        '\',this);" class="fa fa-times-circle delete-tag" ></i></span>';
  ...
  deleteTag = function(lang, tag, element) {
    bootbox.confirm({
      size: "small",
  ...
  setSelectProducts = function(id) {
    // $('#btn_add_'+id).prop("onclick", null).off("click");
    $("#btn_add_" + id).hide();
    $("#btn_added_" + id).fadeIn();
    var html =
      '<li id="added_product_' +
      id +
      '" class="list-group-item selected-product" data-id="' +
      id +
      '"><div class="label-image">' +
      $(".product-image-" + id).html() +
      '</div><div class="label-name">' +
      $(".product-name-" + id).html() +
      "</div>" +
      '<a id="btn_added_' +
      id +
      '" onclick="removeSelectProducts(' +
      id +
      ')" href="javascript:void(0);" class="btn btn-outline-danger"><i class="fa fa-trash"></i></a></li>';
