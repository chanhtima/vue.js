    ...
    <div class="row">
        <div class="form-group col-md-3">
            ...
            <input type="text" class="form-control" name="metadata[en][level]" placeholder="level"
                value="{{ !empty($metadata['en']['level']) ? $metadata['en']['level'] : '' }}" readonly>
        </div>
        <div class="form-group col-md-3">
            <label class="form-label">Module (EN)</label>
            <input type="text" class="form-control" name="metadata[en][module]" placeholder="Module"
                value="{{ !empty($metadata['en']['module']) ? $metadata['en']['module'] : '' }}" readonly>
        </div>
        <div class="form-group col-md-3">
            <label class="form-label">Method (EN)</label>
            <input type="text" class="form-control" name="metadata[en][method]" placeholder="Method"
                value="{{ !empty($metadata['en']['method']) ? $metadata['en']['method'] : '' }}" readonly>
        </div>
        <div class="form-group col-md-3">
            <label class="form-label">Data ID (EN)</label>
            <input type="text" class="form-control" name="metadata[en][data_id]" placeholder="Data ID"
                value="{{ !empty($metadata['en']['data_id']) ? $metadata['en']['data_id'] : '' }}" readonly>
        </div>
    </div>
    <div class="form-group">
        <?= image_upload($id = 2, $name = 'meta_image_en', $label = __('websetting_admin.upload_image_header') . ' Favicon', $image = !empty($metadata['en']['meta_image']) ? $metadata['en']['meta_image'] : '', $size_recommend = '48 x 48px') ?>
    </div>
