...
<div class="tab_content active">
    <div class="row">
        <div class="form-group col-md-3">
            ...
            <input type="text" class="form-control" name="metadata[th][level]" placeholder="Meta title"
                value="{{ !empty($metadata['th']['level']) ? $metadata['th']['level'] : '' }}" readonly>
        </div>
        <div class="form-group col-md-3">
            <label class="form-label">Module (TH)</label>
            <input type="text" class="form-control" name="metadata[th][module]" placeholder="Module"
                value="{{ !empty($metadata['th']['module']) ? $metadata['th']['module'] : '' }}" readonly>
        </div>
        <div class="form-group col-md-3">
            <label class="form-label">Method (TH)</label>
            <input type="text" class="form-control" name="metadata[th][method]" placeholder="Method"
                value="{{ !empty($metadata['th']['method']) ? $metadata['th']['method'] : '' }}" readonly>
        </div>
        <div class="form-group col-md-3">
            <label class="form-label">Data ID (TH)</label>
            <input type="text" class="form-control" name="metadata[th][data_id]" placeholder="Data ID"
                value="{{ !empty($metadata['th']['data_id']) ? $metadata['th']['data_id'] : '' }}" readonly>
        </div>
    </div>
    <div class="form-group">
        <?= image_upload($id = 1, $name = 'meta_image_th', $label = __('websetting_admin.upload_image_header') . ' Favicon', $image = !empty($metadata['th']['meta_image']) ? $metadata['th']['meta_image'] : '', $size_recommend = '48 x 48px') ?>
    </div>
