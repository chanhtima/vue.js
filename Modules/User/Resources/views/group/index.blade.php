...
@section('content')
    ...
    <div class="page-header">
        ...
        @if (mwz_roles('admin.product.brand.add'))
            <div class="ml-auto">
                <div class="input-group">
                    <a href="{{ route('admin.product.brand.add') }}" class="btn btn-secondary text-white mr-2 btn-sm"
                        data-toggle="tooltip" title="" data-placement="bottom" data-original-title="Add">
                        <span>
                            <i class="fa fa-plus" aria-hidden="true"></i> {{ __('product::module.action.add') }}
                        </span>
                    </a>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="brand-datatable" class="table table-bordered key-buttons text-nowrap w-100">
                            <thead>
                                <tr>
                                    <th class="border-bottom-0" width="5%">{{__('product::brand.datatable.sequence')}}</th>
                                    <?php if($config['image']){ ?>
                                    <th class="border-bottom-0" width="15%">{{__('product::brand.datatable.image')}}</th>
                                    <?php } ?>
                                    <th class="border-bottom-0" width="">{{__('product::brand.datatable.name')}}</th>
                                    <th class="border-bottom-0" width="15%">{{__('product::brand.datatable.updated_at')}}</th>
                                    <th class="border-bottom-0" width="5%">{{__('product::brand.datatable.manage')}}</th>
                                </tr>
                            </thead>
                            <tbody>
                            </tbody>
